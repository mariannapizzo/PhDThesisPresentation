...
%% --- SLIDE 1 (accorpata): GOAL, DATASET, METRICHE ---
\begin{frame}{Study 1: Goal, Dataset and Metrics}
	...
	\textbf{\textcolor{gbAqua}{Five Software Packages Tested}} (default parameters, no expert tuning)
	
	3DF Zephyr, Metashape, Colmap, Meshroom, Regard3D
	...
	\begin{columns}[t]
		...
		\begin{itemize}
			\setlength\itemsep{0em}
			\item Synthetic dataset with known ground truth (.obj)
			\item 5 objects, 2 lighting conditions (fixed/moving sunlight)
		\end{itemize}
		...
		\begin{itemize}
			\setlength\itemsep{0em}
			\item Geometric accuracy: C2C, C2M (CloudCompare)
			\item Processing time, failure rate, ease of use
		\end{itemize}
	\end{columns}
...
%% --- SLIDE 2 (accorpata): RISULTATI E FINDINGS ---
\begin{frame}{Study 1: Results and Findings}
	\begin{center}
	...
	\alert{3DF Zephyr} offers the best accuracy/time trade-off, with low failure rate: recommended for non-expert users reconstructing large sets of objects.
	
	\vspace{0.15cm}
	
	\alert{Colmap} achieves the lowest error but is by far the slowest (nearly 70 min per object) $\rightarrow$ \textbf{excluded from Study 2} due to processing time.
